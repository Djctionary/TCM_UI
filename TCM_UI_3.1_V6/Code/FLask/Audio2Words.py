...
class Video2Audio:
    ...
    def find_latest_recorded_video(self, directory, keyword="患者自述"):
        file_pattern = re.compile(r'^(\d+)_{}(?: \((\d+)\))?\.mp4$'.format(keyword))
        ...
        # 从文件名中提取视频ID
        video_ids = [int(file_pattern.match(file).group(1)) for file in matching_files]

        # 获取包含 "keyword" 的文件
        keyword_files = [file for file in matching_files if keyword in file]

        # 如果包含关键词的文件，直接返回它的路径
        if keyword_files:
            return os.path.join(directory,
                                max(keyword_files, key=lambda x: os.path.getctime(os.path.join(directory, x))))

        # 获取最大视频ID对应的文件
        max_video_id_file = max(zip(video_ids, matching_files), key=lambda x: x[0])

        return os.path.join(directory, max_video_id_file[1])
    ...
    def convert_video_to_audio(self):
        try:
            ...
            command = ['ffmpeg', '-i', self.video_path, '-vn', '-acodec', 'pcm_s16le', '-ar', '44100',
                       output_audio_path]
...
class RequestApi(object):
    ...
    def get_result_request(self, taskid):
        ...
        # 将结果保存到文件
        with open(config.get('Audio2Words_Paths', 'words_result_path'), 'w', encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    video_path = config.get('Audio2Words_Paths', 'video_path')
    output_audio_path = config.get('Audio2Words_Paths', 'output_audio_path')
