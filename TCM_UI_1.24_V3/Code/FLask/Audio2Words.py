...
class Video2Audio:
    ...
    def find_latest_recorded_video(self, directory):
        ...
        latest_file = max(matching_files, key=lambda x: int(file_pattern.match(x).group(1) or 0))

        # 如果只有 bestfile，直接返回它的路径
        if f'{base_filename}' in matching_files and latest_file is None:
            return os.path.join(directory, f'{base_filename}')
